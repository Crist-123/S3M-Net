...
try:
    from evaluate_stereo import validate_things
except ImportError:
    validate_things = None
...
def train(args):
    ...
    while should_keep_training:
        ...
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            optimizer.zero_grad()
            image1, image2, flow, valid,semantic = [x.cuda() for x in data_blob]

            assert model.training
            flow_predictions,semantic_pre = model(image1, image2, iters=args.train_iters)
            assert model.training

            stereo_loss, metrics = sequence_loss(flow_predictions, flow, valid)
            loss_fn = SegmantationLoss()
            semantic_loss=loss_fn(semantic_pre,semantic.long())
            loss=stereo_loss+semantic_loss
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                ...
                if validate_things is not None:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', type=int, nargs='+', default=[320, 640], help="size of the random image crops used during training.")
    ...
    parser.add_argument('--shared_backbone', default=True,action='store_true', help="use a single backbone for the context and feature encoders")
